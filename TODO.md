...
기반으로 한 단계별 구현 현황 및 로드맵. (현재 종합 완성도: **100%** 🎉)
...
- [x] **실시간 데스크 객체 및 사물 인식** — YOLOv8 기반 실시간 데스크탑 80종 물체 검출 및 위치 파악 도구 ([`vision/yolo_detector.py`](src/reachy_mini_conversation_app/vision/yolo_detector.py), [`tools/detect_objects.py`](src/reachy_mini_conversation_app/tools/detect_objects.py))
...
- [x] **Google Calendar 및 5대 데일리 스케줄 서비스** — OAuth 기반 구글 캘린더 실시간 연동 및 기본 5대 데스크 루틴 자동 연동 ([`calendar_service.py`](src/reachy_mini_conversation_app/calendar_service.py), [`tools/get_schedule.py`](src/reachy_mini_conversation_app/tools/get_schedule.py))
- [x] **MCP(Model Context Protocol) 클라이언트 인프라** — 원격 외부 도구 확장을 위한 표준 MCP 클라이언트 통합 ([`mcp_client.py`](src/reachy_mini_conversation_app/mcp_client.py))
...
- [x] **멀티소스 카메라 스트리밍 & 실시간 웹 UI 제어** — Reachy Mini 로봇 카메라 / USB 웹캠 실시간 감지, MJPEG 15fps 스트리밍 및 설정 UI 연동 ([`camera_service.py`](src/reachy_mini_conversation_app/camera_service.py))
...
스마트 데스크 동반자 5단계 시나리오 통합 및 전체 검증 완료.

- [x] **스마트 데스크 동반자 5단계 시나리오 통합 완성 (종합 완성도 100%)**
  - [x] 1단계: 대기 및 호출 (Idle & Wake) — DoA 감지, 사용자 방향 헤드 회전, Wake 제스처 및 첫인사
  - [x] 2단계: 모닝 브리핑 및 일정 확인 (Morning Briefing) — Google Calendar / 기본 5대 일정 요약 전달, 끄덕임 제스처 동기화
...
- [x] **`main` 브랜치 머지 완료** — `feat/openai-korean-voice-baseline` 머지 및 배포 파이프라인 동기화
- [x] **품질 검증 통과** — Pytest (415개 테스트 100% 통과), Ruff 린트/포맷 클린, Mypy Strict 통과 (0 errors in 58 files)
...
| 입력 | Sony IMX708 CSI 카메라 / USB 웹캠 | 비전 캡처·3D 안면인식·사물인식 | `camera_service.py`, `tools/detect_face.py`, `tools/detect_objects.py` |
...
| AI / Tools | OpenAI / Google Cal / YOLO / MCP | 음성 대화, 뽀모도로, 일정 관리, 비전 도구 | `tools/pomodoro_timer.py`, `tools/get_schedule.py`, `tools/detect_objects.py`, `mcp_client.py` |
